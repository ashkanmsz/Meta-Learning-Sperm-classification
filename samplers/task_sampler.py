...
Original file is located at
    https://colab.research.google.com/drive/1iuJqob_qFnsjUYcM6MqLNpvysYlz-5u7
"""
...
from sperm.dataset.few_shot_dataset import FewShotDataset
...
class TaskSampler(Sampler):
    ...
    def __init__(
        ...
        if len(set(dataset.targets)) == 4:
          self.multiLabel = 4
          self.labels = [[0,1],[2,3]]
          self.distributions = [0.5,0.5]
        
        elif len(set(dataset.targets)) == 6:
          self.multiLabel = 6
          self.labels = [[0,1],[2,3],[4,5]]
          self.distributions = [0.3,0.3,0.3]
            ...
            if self.multiLabel == 4 :
              yield torch.cat(
                  [
                      ...
                      for sublist in random.choices(self.labels ,self.distributions , k=1) for label in sublist
                  ]
              ).tolist()
              
            elif self.multiLabel == 6 :
              yield torch.cat(
                  [
                      torch.tensor(
                          random.sample(
